...
const TYPE_CHART: Partial<Record<TypeId, Partial<Record<TypeId, number>>>> = {
...
export function getTypeEffectiveness(attackType: TypeId, defenderTypes: TypeId[]): number {
  let multiplier = 1;
  for (const defType of defenderTypes) {
    const chart = TYPE_CHART[attackType];
    if (chart && chart[defType] !== undefined) {
      multiplier *= chart[defType] as number;
    }
  }
  return multiplier;
}
